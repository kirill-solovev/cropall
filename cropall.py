...
class MyApp(Tk):
    ...
    def __init__(self):
        ...
        self.selection_mode_dropdown = OptionMenu(
            self.controls, self.selection_mode, "click-drag", "scroll"
        )
        ...
        self.inputs += [
            Checkbutton(
                self.controls, text="Fix Aspect Ratio", variable=self.restrictRatio
            )
        ]
        ...
        self.buttons += [Button(self.controls, text="Next", command=self.__next__)]
        ...
        self.inputs += [
            Checkbutton(
                self.controls, text="Perfect Pixel Ratio", variable=self.restrictSizes
            )
        ]
        ...
        while self.current < len(self.files) and os.path.exists(
            ...
            print("Skipping " + str(self.files[self.current]) + ". Already cropped.")
            self.current += 1
        self.current += 1
        self.previous()
    ...
    def getRealBox(self):
        ...
        if self.selection_mode.get() == "click-drag":
            top_left = (
                self.selection_tl_x * imw / prevw,
                self.selection_tl_y * imh / prevh,
            )
            top_left = (max(top_left[0], 0), max(top_left[1], 0))
            bottom_right = (
                self.selection_br_x * imw / prevw,
                self.selection_br_y * imh / prevh,
            )
            bottom_right = (
                max(top_left[0] + 1, min(bottom_right[0], imw)),
                max(top_left[1] + 1, min(bottom_right[1], imh)),
            )
            ...
            if self.restrictRatio.get() == 1:
                if (
                    float(w) / float(h) > self.aspectRatio
    ...
    def copy(self):
        c = (
            'copy "'
            + os.path.join(self.inDir, self.currentName)
            + '" "'
            + os.path.join(self.outDir, self.currentName)
            + '"'
        )
        print(c)
        shutil.copy(
    ...
    def update_preview(self, widget):
        if self.item:
            ...
            preview.thumbnail(
                self.image.size, Image.ANTIALIAS
